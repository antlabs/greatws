...
func Test_ClientOption(t *testing.T) {
	...
	t.Run("ClientOption.WithClientHTTPHeader", func(t *testing.T) {
		...
		con, err := Dial(url, WithClientHTTPHeader(http.Header{
			"A": []string{"A"},
		}), WithClientCallback(&testDefaultCallback{}), WithClientMultiEventLoop(m))
		if err != nil {
			t.Error(err)
			return
		}
	...
	t.Run("6.1 Dial: WithClientBindHTTPHeader and echo Sec-Websocket-Protocol", func(t *testing.T) {
		...
		ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			_, err := Upgrade(w, r, WithServerMultiEventLoop(m))
			if err != nil {
				t.Error(err)
			}
		}))
		...
		url := strings.ReplaceAll(ts.URL, "http", "ws")
		h := make(http.Header)
		con, err := Dial(url, WithClientBindHTTPHeader(&h), WithClientMultiEventLoop(m), WithClientHTTPHeader(http.Header{
	...
	t.Run("6.2 DialConf: WithClientBindHTTPHeader and echo Sec-Websocket-Protocol", func(t *testing.T) {
		...
		url := strings.ReplaceAll(ts.URL, "http", "ws")
		h := make(http.Header)
		con, err := DialConf(url, ClientOptionToConf(WithClientBindHTTPHeader(&h), WithClientMultiEventLoop(m), WithClientHTTPHeader(http.Header{
			"Sec-WebSocket-Protocol": []string{"token"},
		})))
		if err != nil {
			t.Error(err)
	...
	// 	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	// 		_, err := Upgrade(w, r, WithServerOnMessageFunc(func(c *Conn, o Opcode, b []byte) {
	// 			c.WriteMessage(o, b)
	// 			c.Close()
